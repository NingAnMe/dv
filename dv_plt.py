...
def get_DV_Font(fontName=u'winhei.ttf'):
    '''
    载入字体
...
def add_colorbar_below(fig, pos_xl, pos_xr, pos_y, valmin, valmax,
                       cmap=colormap_blue2red(), colorbar_height=0.03,
                       fmt="%d", extend="neither", bounds=None, unit=None,
                       font=get_DV_Font(), fontSize=8):

    '''
    在fig下方添加colorbar
    '''
    colorbar_ax = fig.add_axes([pos_xl, pos_y,
                                pos_xr - pos_xl, colorbar_height])
    ...
    def __init__(self, fig=None, ax=None, **kwargs):
        ...
        if ax is None:
            if "subplot" in kwargs:
                self.ax = self.fig.add_subplot(kwargs["subplot"])
#                 if kwargs["subplot"] == 111:
#                     self.show_tight = True
            else:
...
#         return self.fig.add_subplot(self.__subplot)
        return self.ax
    ...
    @staticmethod
    def singleline(x1, x2, y1, y2, color, linewidth, zorder=0):
        '''
        画一条直线
        '''
        plt.plot([x1, x2], [y1, y2], color=color, linewidth=linewidth, zorder=zorder)
...
#         print point_bl, point_tr, x_toedge
    ...
    def draw(self):
        ...
        # text
        if self.title != '':
            tt = ax.set_title(self.title, fontproperties=self.font, fontsize=self.fontsize_title)
            tt.set_y(self.title_pos)  # set gap space below title and subplot
        if self.xlabel != '':
            plt.xlabel(self.xlabel, fontproperties=self.font, fontsize=self.fontsize_label)
        if self.ylabel != '':
            plt.ylabel(self.ylabel, fontproperties=self.font, fontsize=self.fontsize_label)
        #
        self.custom_style()
        #
        ...
        if mpl.__version__.split(".")[0] < 2:
            ...
            plt.tick_params(which='both', width=0.6)
...
#             plt.tight_layout()
    ...
    def suptitle(self, suptitle, fontName=None, fontSize=16):
        if fontName is not None:
            font = get_DV_Font(fontName)
        else:
            font = get_DV_Font()
        font.set_size(fontSize)
        self.fig.suptitle(suptitle, fontproperties=font, fontsize=fontSize)

    def title_center(self, title):
        ax = self.get_main_ax()
        ax.set_title(title, fontproperties=self.font, fontsize=self.fontsize_title)

    def title_left(self, title):
        ax = self.get_main_ax()
        ax.set_title(title, fontproperties=self.font, fontsize=self.fontsize_title, loc='left')

    def title_right(self, title):
        ax = self.get_main_ax()
        ax.set_title(title, fontproperties=self.font, fontsize=self.fontsize_title, loc='right')

    def addIcon(self):
        pass

    def savefig(self, figPath, dpi=150):
        '''
        保存图像
        '''
        self.draw()
...
class dv_line_chart(dv_base):
    ...
    def easyplot(self, x, y, color, name, marker='o-', markersize=4.2, mec=None, alpha=1):
        '''
        画折线
        marker = 'o-' or '-'
        '''
        ...
        self.set_xminmax(x)
        self.set_yminmax(y)
        plt.plot(x, y, marker, mfc=mfc, mec=mec, color=color,
                 ms=markersize, mew=mew, lw=self.line_width,
                 label=name, alpha=alpha)

    def twinx(self, x, y, color, name, ylabel='', marker='o-', markersize=5, mec=None, alpha=1):
        mfc = color
        if mec is None:
            mew = 0
        else:
...
#             self.leg_len = max(str_len(name), self.leg_len)
            ...
            self.set_yminmax(y)
            plt.bar(x_pos, y, width, color=color,
                    align='center', linewidth=0.1,
                    bottom=bottom,
                    yerr=err, alpha=alpha)
            plt.xticks(x_pos, x)

        elif isinstance(y[0], basestring):
#             self.leg_len = max(str_len(name), self.leg_len)
            y_pos = np.arange(len(y))
            self.set_xminmax(x)
            plt.barh(y_pos, x, width, color=color,
                    align='center', linewidth=0.1,
                    left=bottom,
                    yerr=err, alpha=alpha)
            plt.yticks(y_pos, y)
...
class dv_imshow(dv_base):
    ...
    def addRects(self, color_list, color_names, pad="2%", betweenRects=0.04, maxRect=8):
        ...
        for i in xrange(len(color_list)):
            ...
            plt.text(x1, y1, text, ha="center", va="top", size=8,
...
#     p.annotate(strlist, color='m', fontsize=15)
#
#     p.savefig('sd.png')
#
    # 折线 -------------------
#     p = dv_line_chart()
#     p.easyplot(x1, y1, 'c', u'w', "x-", mec="c")
#     p.easyplot(x2, y2, 'b', u'a', mec="orange")
#     p.title = u'折线图'
#     p.xlabel = u'X轴'
#     p.ylabel = u'Y轴'
# #     p.grid(True)
...
# #     p.y2lim_max = 8
# #     p.y2lim_min = -8
# #     p.twinx(x1, y2, 'r', "std", ylabel=u"Y轴2", marker='-')
#
#     p.savefig('zx.png')
