...
'''
import os
from dv_plt import dv_base, str_len, EDGE_LW, colormap_blue2red, COLOR_Darkgray, get_DV_Font, add_colorbar_horizontal
import numpy as np
...
#         self.show_tight = False
    ...
    def drawOnMap(self, m, alpha):
        ...
        if ptype == 'pcolormesh':
            cs = m.pcolormesh(x1, y1, var, norm=self.norm, rasterized=True, cmap=self.colormap, zorder=zorder, alpha=alpha)
        elif ptype == 'pcolor':
            cs = m.pcolor(x1, y1, var, norm=self.norm, cmap=self.colormap, zorder=zorder, alpha=alpha)
        elif ptype == 'imshow':
            cs = m.imshow(var, interpolation="spline16", cmap=self.colormap, zorder=zorder, alpha=alpha)
        elif ptype == 'contour':
            cs = m.contour(x1, y1, var, 10, linewidths=0.5, cmap=self.colormap, zorder=zorder, alpha=alpha)
        elif ptype == 'contourf':
...
#                                  linewidth=0.2, color=color_contry)
    ...
    def add_landmark(self, lonlatlocation, color="r", marker="star", markersize=70, fontsize=7.5, yplus=0):
    ...
    #         circle1 = mpl.patches.Star.Circle((xpt, ypt), 6, color=RED, ec=EDGE_GRAY, lw=0.3)
            self.m.scatter(xpt, ypt, marker=marker, s=markersize, color=color, edgecolors="#999999", lw=0.1, zorder=10)
            fnt = get_DV_Font("SourceHanSansCN-Medium.otf")

            self.ax.annotate(location, xy=(xpt, ypt + yplus), ha="center", fontproperties=fnt, size=fontsize, color=COLOR_Darkgray)
